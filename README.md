...
- Удаление элементов из selection через `/api/selection/remove` (идемпотентность, валидация, множественное удаление)
...
Удалить элементы из selection (выбранных элементов). Удаление применяется немедленно в in-memory хранилище.
...
**Формат ответа:**
- `ok` (boolean) - статус успешности операции
- `removed` (number[]) - массив реально удалённых ID
- `selectedIds` (number[]) - обновлённый массив выбранных ID
- `order` (number[]) - обновлённый упорядоченный массив ID

**Примечания:**
- Удаление применяется немедленно (без батчинга)
- Операция идемпотентна: повторный запрос с теми же ID не ломает порядок и возвращает пустой массив `removed`
- Если ID не существует в selection, он игнорируется (не вызывает ошибку)
- Удаление очищает элемент как из `selectedIds`, так и из `order`

**Пример повторного удаления:**
```json
{
  "ok": true,
  "removed": [],
...
**Ошибки:**
- `400` - Неверный формат данных (`INVALID_INPUT`, `INVALID_IDS`)
- `500` - Внутренняя ошибка сервера
...
## Изменения по требованию: удаление в правой колонке

ТЗ не требовало функцию удаления в правой колонке, но по UX-дополнению она реализована.

**Реализовано:**
- `POST /api/selection/remove` — немедленное удаление выбранных ID из in-memory storage
- UI: кнопка "Удалить" (✖) у каждого элемента правой колонки
- Toast-уведомления для обратной связи с пользователем
- Поддержка удаления одного или нескольких элементов одновременно

**Поведение:**
- Удаление идемпотентно (безопасно повторять)
- Изменения сразу видны пользователю и сохраняются в memoryStore
- Удалённые элементы автоматически возвращаются в левую колонку (при следующем polling/fetch)
- Логирование всех операций удаления на сервере

**UI/UX:**
- Кнопка удаления с визуальной обратной связью (hover эффект)
- Toast-уведомления: "Элемент удалён", "Ничего не удалено", "Ошибка при удалении"
- Немедленное обновление UI без ожидания polling
